...
data_path = './data/Dataset_1_NCA_battery/'
files = os.listdir(data_path)  #
files = [file for file in files if any(substring in file for substring in [ 'CY35-05_1', 'CY45-05_1'])]#
...
for prefix in bat_prefix:
    ...
    bats_dic[prefix] = {'rul': cyc_rul,
                        'dq': cyc_dq,
                        'data': cyc_v}
    save_obj(bats_dic, './data/NCA_data/' + prefix)


pkl_list = os.listdir('./data/NCA_data/')
#
target_strings = [ 'CY35-05_1', 'CY45-05_1']#
...
def get_xy(name):
    A = load_obj(f'./data/NCA_data/{name}')[name]  #
    A_dq = A['dq']
    A_df = A['data']
    A_rul = A['rul']
...
all_lbl = []
print('----init_train----')
if os.path.exists('./data/NCA_data/nmc_all_loader.pkl'):
    ...
    save_obj(all_loader, './data/NCA_data/nmc_all_loader')