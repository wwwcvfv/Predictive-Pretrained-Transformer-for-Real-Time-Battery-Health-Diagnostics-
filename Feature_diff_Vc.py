...
for i in cycle_train:
    if '20' in i:
        ...
        vc_ = []
        cycle = []
        for j in range(11, 720):#bat_life
            if key[0] == 'a':
                temp = temp1
                batch = batch1
            # elif key[0] == 'b':
        ...
        for i, (v, cycle_num) in enumerate(zip(df_q ,cycle)):
            color = cmap(norm(cycle_num))
            plt.plot(x_data, v, color=color, linewidth=2)

#
ax.set_xlim(0, 100)
ax.set_ylim(np.min(df_q), np.max(df_q))
#
...
ax.set_ylabel('\u0394Vi-10 (V)', fontsize=35, family='Times New Roman')
...
plt.savefig('Figure/f-diffv.tiff', dpi=800, format="tiff")
plt.show()
