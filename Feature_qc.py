...
for i in cycle_train:
    if '20' in i:
        ...
        for i, (q, cycle_num) in enumerate(zip(qc, cycle)):
            color = cmap(norm(cycle_num))
            plt.plot(x_data, q, color=color, linewidth=2)

#
ax.set_xlim(0, 100)
ax.set_ylim(np.min(qc), np.max(qc))
#
plt.tick_params(axis='x', labelsize=35)  #
plt.tick_params(axis='y', labelsize=35)  # 设
...
ax.set_ylabel('Charge capacity (Ah)', fontsize=35, family='Times New Roman')
#
...
plt.savefig('Figur/f-q.tiff', dpi=800, format="tiff")
plt.show()
